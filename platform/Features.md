...
| Per-model consensus gates | `consensus_min_confidence` / `consensus_min_margin` overridable per model in `model_override`; model excluded from fusion if its individual logit fails gate |
...
| Feature | Description |
|---|---|
| OpenTelemetry instrumentation | All Python services emit traces + metrics + logs via OTLP; logs also reach Loki via Docker filelog receiver |
