...
| `VAULT_MASTER_KEY_1` | — | ⬜ | Previous KEK (monotonic: `_1` = previous, `_2` = older). Required during rotation; remove after `alphakey rotate-master-key` completes. See **KEK Rotation** below. |
...
ALPHATRADE_API_KEY: ${ALPHATRADE_API_KEY:-}
JWT_ISSUER: ${JWT_ISSUER:-alphakey}
JWT_AUDIENCE: ${JWT_AUDIENCE:-alphakey}
MLFLOW_TRACKING_URI: http://mlflow:5000
MLFLOW_S3_ENDPOINT_URL: http://minio:9000
...
3. Restart `alphakey-api` (`docker compose up -d alphakey-api`) so both keys are live
4. Dry-run: `docker compose exec alphakey-api alphakey rotate-master-key --dry-run`
5. Execute: `docker compose exec alphakey-api alphakey rotate-master-key`
   - Idempotent — rows already on the new version are skipped; safe to re-run
6. Remove `VAULT_MASTER_KEY_1` (and any older `_N` vars) from `.env` and restart

> [!warning] Do not remove `VAULT_MASTER_KEY_1` before step 5 completes — DEKs still wrapped under the old key will become unreadable.
