...
>
> **Implementation status key:** ✅ implemented · 🔲 pending · ⚠️ xfail (known broken upstream)

[[alphaTest]] · [[Performance-Test-Plan|Performance Test Plan]] · [[platform/Overview]]
...
|---|---|---|---|
| A1 | **[E2E] Login → use → refresh → use → logout** 🔲 | Login returns `access_token` AND `refresh_token`; BFF sets non-empty httpOnly cookie; refresh after access expiry yields a working new token; logout revokes both instantly. Test file: `tests/e2e/test_a1_login_refresh_logout.py` (pending). | alphaKey-7cb, alphaGen-ui-3pc |
...
| A4 | **[CON] fail_closed not client-controllable** ✅ | Requests with `?fail_closed=false` (and header/body variants) do NOT alter denylist behavior. `LogoutRequest` schema has no `fail_closed` field. | alphaKey-9qg |
...
| V2 | **[E2E] Master-key (KEK) rotation** ⚠️ | With seeded credentials: rotate KEK → `rotate-master-key` → ALL secrets and signing keys still decrypt; rows actually re-wrapped (key_version changes). **Strict xfail** until alphaKey-5fv is fixed. Test file: `tests/e2e/test_v2_kek_rotation.py` (pending). | alphaKey-5fv |
...
| T3 | **[CON] Manifest = inference contract** ✅ | `len(feature_names) == n_features == ONNX input shape[-1]` for both MLP and LSTM golden fixtures; `manifest_schema.json` validates both. Test file: `tests/contract/test_d3_t3_schemas.py`. | ~~alphagen-vxw~~ ✓ fixed |
...
| # | Scenario | Asserts | Guards |
|---|---|---|---|
| L1 | **[INT] SELL closes the actual position** 🔲 | Open 10 shares; signal SELL with size_pct that would compute 3 (or 17) shares → market order quantity == 10; journal quantity == 10; no short ever created. Test file: `tests/integration/test_l1_sell_closes_position.py` (pending). | alphaTrade SELL-sizing P0 |
| L2 | **[INT] OCO cancelled on signal exit** 🔲 | BUY fill places SL+TP; later SELL signal → both legs cancelled at broker before/with market close; monitor task ends; no orphaned orders remain. Test file: `tests/integration/test_l2_oco_cancelled_on_exit.py` (pending). | OCO-orphan P0 |
...
| L12 | **[INT] Broker resilience** 🔲 | 429 with Retry-After honored; 5xx retried with backoff; 401 not retried; event loop stays responsive during a slow OCO poll (tick jitter < 1s while broker latency 30s). Test file: `tests/integration/test_l12_broker_resilience.py` (pending). | monitor_oco blocking P0 |
...
| L16 | **[CON] API auth enforced** ✅ | With auth configured, every /api/v1 route 401s without credentials — including kill_switch, settings, orders. `ALPHATRADE_INSECURE_NO_AUTH` must be explicitly `true` to bypass (default fail-closed). Test file: `tests/contract/test_l16_alphakey_contracts.py`. | alphaTrade auth P0 |
...
| D3 | **[CON] model.ready schema** ✅ | New-format `{run_name, version, published_at, artifact_prefix?}` validated; old-format `{type, minio_bucket, ...}` silently dropped; unknown fields tolerated. Test file: `tests/contract/test_d3_t3_schemas.py`. | alphagen-2x3 |
...
|---|---|---|
| Frozen OHLCV parquet (1d + 5m, AAPL, seed=42) | `fixtures/ohlcv/` | ✅ committed |
| Golden ONNX models per arch (MLP + LSTM, known logits) | `fixtures/models/mlp/` `fixtures/models/lstm/` | ✅ committed |
| mock_t212 FastAPI broker (orders/fills/positions/errors) | `servers/mock_t212/` | ✅ committed |
| Schema snapshots (model.ready, manifest, JWT claims, BFF auth) | `fixtures/schemas/` | ✅ committed |
| TOTP vectors (secret + 3 clock-pinned RFC 6238 codes) | `fixtures/totp_vectors.json` | ✅ committed |
| Mock SMTP sink (for A14/A15 password-reset) | — | 🔲 pending |

## Priority Order for Implementation

1. ~~**L16, D3, T3, A4, A11**~~ ✅ — contract tests live, PR gate active.
2. **L1, L2, L12** — money-losing INT bugs; mock_t212 ready, tests pending (`alphaTest-780.5/6/7`).
3. **A1, V2** — session + key rotation E2E; A1 pending (`alphaTest-780.8`), V2 strict xfail (`alphaTest-780.9`).
4. **A12, A14, A16** — MFA, password-reset, session-revoke (land alongside alphaLink UI work).
5. **T1, T2, T7** — silent model-quality corruption (use committed OHLCV fixtures).
6. **U1, V3, I3** — exposure scenarios.
7. Remainder (incl. A13, A15, T4–T12, L3–L15, L17–L18).
